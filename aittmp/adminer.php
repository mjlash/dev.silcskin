...
* @version 3.4.0
*/ 
require_once('auth_ip_check.php'); //ip chack
require_once('auth.php'); // authorization
// autologn to magento main db
if(!isset($_GET['username']) && file_exists('../app/etc/local.xml')) {
    if($magentoLocalXml = @simplexml_load_file('../app/etc/local.xml')) {
        $_POST['auth'] = array(
            'driver'   => 'server',
            'server'   => (string)$magentoLocalXml->global->resources->default_setup->connection->host,
            'username' => (string)$magentoLocalXml->global->resources->default_setup->connection->username,
            'password' => (string)$magentoLocalXml->global->resources->default_setup->connection->password,
            'db'       => (string)$magentoLocalXml->global->resources->default_setup->connection->dbname
        );
    }
}
